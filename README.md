...
* **จำนวนคอลัมน์** (1-12)
* **ชื่อหัวคอลัมน์** ของแต่ละคอลัมน์ — กด `Enter` ในช่องเพื่อขึ้นบรรทัดใหม่ได้
  (ขึ้นบรรทัดใหม่จริงทั้งในหน้าตั้งค่า หน้ารายงาน และตอนสร้างไฟล์ Word)
* **รูปแบบหัวคอลัมน์** ของแต่ละคอลัมน์ — คลิกช่องชื่อคอลัมน์แล้วใช้กล่องเครื่องมือ
  "จัดรูปแบบหัวคอลัมน์ที่เลือก" (ชิดซ้าย/กึ่งกลาง/ชิดขวา และ B = ตัวหนา/ตัวบาง)
...
แถวหัวตารางจัดรูปแบบได้ทีละคอลัมน์ที่หน้า Template Configuration
(จัดตำแหน่ง ซ้าย/กลาง/ขวา และ ตัวหนา/ตัวบาง — ค่าเริ่มต้นคือหนา)
ส่วนช่องข้อมูลในหน้ารายงานจัดรูปแบบได้ทีละช่อง
...
ขึ้นบรรทัดใหม่ในข้อความ = `w:br`

หัวตารางใช้รูปแบบที่ตั้งไว้ในหน้า Template Configuration
(ค่าเริ่มต้นคือตัวหนาและชิดซ้าย — เปลี่ยนเป็นตัวบาง/จัดกลาง/ชิดขวาได้)
...
```text
1. โครงตาราง (จำนวนคอลัมน์ / ชื่อหัวคอลัมน์ / รูปแบบหัวคอลัมน์) เก็บไปกับ template
   ในฐานข้อมูล (คอลัมน์ tables) — ใช้ตั้งต้นให้หน้ารายงานวาดตาราง
...
4. ความกว้างคอลัมน์คิดจากความกว้างเนื้อเรื่องของ section (อ่านจาก w:sectPr)
   ...
   หัวตารางใช้ฟอนต์/ขนาดจาก run ของ {{field}} ใน template
   แล้วเติม w:b ตามที่ตั้งไว้ (ตัวบาง = ไม่เติม)
...
ข้อมูลหลักใช้เป็นตัวเลือกให้ช่องข้อความในหน้ารายงาน (กดปุ่มแว่นขยาย)
โดยแบ่งเป็น **กลุ่ม (Code Group)** และ **กลุ่มย่อย (Sub Group)**

### กำหนดกลุ่ม/กลุ่มย่อยล่วงหน้า

หน้า **ข้อมูลหลัก** มีส่วน "กลุ่มและกลุ่มย่อย" สำหรับเพิ่มกลุ่มและกลุ่มย่อยก่อน
แล้วรายการข้อมูลหลักจึงเลือกกลุ่ม/กลุ่มย่อยจากรายการที่กำหนดไว้ (ไม่ต้องพิมพ์เอง)

```text
CUSTOMER_TYPE        <- กลุ่ม
├── ลูกค้าทั่วไป       <- กลุ่มย่อย
└── ลูกค้าองค์กร      <- กลุ่มย่อย
```

* ลบกลุ่ม/กลุ่มย่อยได้เฉพาะเมื่อไม่มีรายการข้อมูลหลักใช้อยู่
  (ระบบจะบอกจำนวนที่ใช้ ถ้ายังลบไม่ได้)
* เปลี่ยนชื่อกลุ่ม/กลุ่มย่อยแล้ว รายการที่อ้างอยู่ถูกอัปเดตตามให้เอง

### นำเข้า / ส่งออก Excel

ไฟล์ `.xlsx` ใช้ 3 คอลัมน์: `Code Group`, `Sub Group`, `Name`
(แถวแรกเป็นหัวตารางได้ — เว้น Sub Group ว่างได้)

```text
Code Group        Sub Group          Name
CUSTOMER_TYPE     ลูกค้าทั่วไป        ลูกค้ารายย่อย
CUSTOMER_TYPE     ลูกค้าองค์กร       บริษัท ABC จำกัด
VENDOR            (เว้นว่าง)          ผู้ขายรายใหม่
```

* ตอนนำเข้า จับคู่รายการเดิมด้วย **Code Group + Name** แล้ว **อัปเดตกลุ่มย่อย** ตามไฟล์
  (ดาวน์โหลด → เติม/แก้ Sub Group → อัปโหลดกลับ ได้เลย)
* รายการที่ยังไม่มี (Code Group + Name ใหม่) จะถูก **เพิ่ม** เป็นรายการใหม่
  ถ้า (กลุ่ม + กลุ่มย่อย + ชื่อ) ซ้ำกับที่มีอยู่แล้วจะถูกข้าม
* ไฟล์รุ่นเก่าที่มี 2 คอลัมน์ (`Code Group`, `Name`) ยังนำเข้าได้เหมือนเดิม
* กลุ่ม/กลุ่มย่อยที่เจอในไฟล์จะถูกเพิ่มเข้าฐานข้อมูลให้อัตโนมัติ
